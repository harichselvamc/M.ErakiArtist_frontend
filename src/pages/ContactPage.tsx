          ...
          >
            ...
            <form className="space-y-6" onSubmit={handleSubmit}>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-sm font-medium mb-2">Your Email <span className="text-red-500">*</span></label>
              ...
              <div>
                ...
                <input type="text" className="input-field" placeholder="What is this regarding?" value={subject} onChange={(e) => setSubject(e.target.value)} required />
              </div>
              <div>
                <label className="block text-sm font-medium mb-2">Your Message <span className="text-red-500">*</span></label>
                <textarea className="textarea-field" rows={5} placeholder="Tell us about your project or question..." value={message} onChange={(e) => setMessage(e.target.value)} required />
              </div>
              <button type="submit" disabled={isSending} className={`btn btn-primary w-full flex items-center justify-center ${isSending ? 'cursor-not-allowed opacity-60' : ''}`}>
                {isSending ? 'Sending...' : (<><Send className="w-4 h-4 mr-2" />Send Message</>)}
              </button>
              {statusMessage && (
                <p className={`text-center mt-4 text-sm ${statusColor === 'green' ? 'text-green-600' : 'text-red-600'}`}>{statusMessage}</p>
              )}
            </form>
          </motion.div>
          ...
          <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.5, delay: 0.2 }}>
            <div className="bg-primary-950 text-white p-8 rounded-lg mb-8">
              ...
              <ul className="space-y-6">
                <li className="flex items-start"><Phone className="w-5 h-5 mr-4 mt-1 text-accent-400" /><div><p className="font-medium">Phone</p><p className="mt-1">+91 7598068106</p></div></li>
                <li className="flex items-start"><Mail className="w-5 h-5 mr-4 mt-1 text-accent-400" /><div><p className="font-medium">Email</p><p className="mt-1">harichselvamc@gmail.com</p></div></li>
                <li className="flex items-start"><MapPin className="w-5 h-5 mr-4 mt-1 text-accent-400" /><div><p className="font-medium">Location</p><p className="mt-1">Tamil Nadu, India</p></div></li>
              </ul>
            </div>
            ...
            <div className="bg-white p-8 rounded-lg shadow-sm border border-primary-100">
              ...
              <a href="https://wa.me/7598068106" target="_blank" rel="noopener noreferrer" className="bg-[#25D366] text-white rounded-lg p-4 flex items-center hover:bg-[#128C7E] transition-colors">
